...
struct BasicImage{
    ...
    BasicImage(IteratorType start, IteratorType end) {
        ...
        decode_pixels<PixelType>(std::ranges::subrange(std::move(start), std::move(end)), std::back_inserter(pixels));
    }
    template<std::ranges::range ByteRange>
    explicit BasicImage(const ByteRange& range): BasicImage{range.begin(), range.end()} {}
    BasicImage(uint32_t width, uint32_t height, ColorType type): header{width, height, PixelType::channel, type} {}
